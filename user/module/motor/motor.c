...
/************** 电机限幅设置 *************/
const int16_t MOTOR_MAX_OUT = 30000; // PWM最大输出值（突破20000限制）
const int16_t MOTOR_MIN_OUT = 0;     // PWM最小输出值

// 速度增益系数（增加电机速度），使用整数运算避免浮点
#define SPEED_GAIN_NUMERATOR 3
#define SPEED_GAIN_DENOMINATOR 2  // 增益 = 3/2 = 1.5
...
{
  // 将油门值映射到PWM范围 (0-1000 -> 0-30000)，并应用速度增益
  // 使用int32_t避免int16_t溢出（30000*1000*3=90000000 > 32767）
  int32_t base_speed = ((int32_t)throttle * MOTOR_MAX_OUT * SPEED_GAIN_NUMERATOR) / (1000 * SPEED_GAIN_DENOMINATOR);

  // 将偏航和横滚值映射到差速范围 (-1000 to 1000 -> -MOTOR_MAX_OUT/4 to MOTOR_MAX_OUT/4)
  int32_t yaw_diff = ((int32_t)yaw * MOTOR_MAX_OUT) / (4 * 1000);
  int32_t roll_diff = ((int32_t)roll * MOTOR_MAX_OUT) / (4 * 1000);
