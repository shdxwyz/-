...
static void quarternion_to_euler(float quat[4], float *roll, float *pitch, float *yaw)
{
	...
	pitch_sin = 2.0f * (quat[0] * quat[3] - quat[1] * quat[2]);
    // 浮点舍入可能令 asin 的输入略微越过 [-1, 1]，从而产生 NaN。
    if(pitch_sin > 1.0f) pitch_sin = 1.0f;
    if(pitch_sin < -1.0f) pitch_sin = -1.0f;
	euler[1] = -asinf(pitch_sin);
	// yaw 分母应为 1 - 2(y^2 + z^2)。原实现误用了 x^2，
	// 使车辆发生俯仰或横滚时 yaw 也会产生虚假的变化。
	euler[2] =  atan2f(2.0f * (quat[0] * quat[1] + quat[2] * quat[3]), 1.0f - 2.0f * (sqy + sqz));
